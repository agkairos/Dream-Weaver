...
export function useLatestCalculation() {
  return useQuery({
    queryKey: [api.calculations.latest.path],
    queryFn: async () => {
      const res = await fetch(api.calculations.latest.path);
      if (!res.ok) throw new Error("Failed to fetch latest calculation");
      return api.calculations.latest.responses[200].parse(await res.json());
    },
  });
}
...
export function useCreateCalculation() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [api.calculations.latest.path] });
    },
  });
}
